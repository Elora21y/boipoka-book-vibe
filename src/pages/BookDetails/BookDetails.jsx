...
const BookDetails = () => {
  ...
  return (
    <div className="my-16 mx-auto w-11/12">
      ...
      >
        <IoMdArrowRoundBack className="text-left" size={30} />
      </button>
      <div className="flex flex-col lg:flex-row justify-center items-center gap-10 ">
        <figure className="bg-base-200 p-10 rounded-lg">
          <img
            className="rounded-lg md:w-[75%] lg:w-[1600px]"
            src={image}
            alt="Book"
          />
        </figure>
        ...
        <div className="space-y-4">
          ...
          <p>
            ...
            <small className="text-gray-600">{review} </small>
          </p>

          <p className="border-b border-gray-300 pb-5">
            <span className="font-semibold">Tag</span> :
            {tags.map((tag, index) => (
              <button
                key={index}
                className="py-1 px-4 rounded-2xl bg-base-200 text-[#23BE0A] ml-4"
              >
                #{tag}
              </button>
            ))}
          ...
          <div className="">
            <button
              onClick={() => handleAddList(singleBook.bookId)}
              className="btn border border-gray-300 mr-4"
            >
              Read
            </button>
            <button
              onClick={() => handleWishList(singleBook.bookId)}
