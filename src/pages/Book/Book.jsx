          ...
          <div className="space-y-4">
            <div>
              {tags.map((tag, index) => (
                <button
                  key={index}
                  className="py-1 px-4 rounded-2xl bg-base-200 text-[#23BE0A] mr-5"
                >
                  {tag}
                </button>
              ))}
            ...
            <div
              className="border-t flex justify-between font-semibold items-center pt-4"
              style={{
                borderImage:
                  "repeating-linear-gradient(to right, #D1D5DB 0 6px, transparent 6px 12px) 1",
                borderImageSlice: 1,
              }}
            >
              <p>{category}</p>
              <p>
                {rating} <GoStar className="inline" size={20} />
              </p>
            </div>
          </div>
        </div>
